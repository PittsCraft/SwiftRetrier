...
public class ConditionalInfallibleRetrier<T>: SingleOutputInfallibleRetrier, SingleOutputConditionalRetrier {
    ...
    public init<P: Publisher<Bool, Never>>(policy: InfallibleRetryPolicy,
                                           conditionPublisher: P,
                                           job: @escaping Job<T>) {
        self.innerRetrier = ConditionalFallibleRetrier(policy: policy.toFallibleRetryPolicy(),
                                                       conditionPublisher: conditionPublisher,
                                                       job: job)
    }

    public func publisher() -> AnyPublisher<Result<T, Error>, Never> {
        innerRetrier.publisher()
            .catch { _ in Empty() }
            .eraseToAnyPublisher()
    }
